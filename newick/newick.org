...
#+end_src
#+begin_src latex
  We use a scanner that wraps the reader provided by the \ty{bufio}
  package. The scanner has a hook for additional fields.
#+end_src
#+begin_src go <<Types, Ch.~\ref{ch:new}>>=
  type Scanner struct {
	  r *bufio.Reader
	  //<<Scanner fields, Ch.~\ref{ch:new}>>
  }
#+end_src
#+begin_src latex
  We import \ty{bufio}.
...
#+end_src
#+begin_src go <<Scanner fields, Ch.~\ref{ch:new}>>=
  text string
#+end_src
#+begin_src latex
  We construct a scanner.
#+end_src
...
#+end_src
#+begin_src latex
  The \ty{Scanner} method \ty{Text} wraps access to the \ty{text}
  fields.
#+end_src
#+begin_src go <<Methods, Ch.~\ref{ch:new}>>=
...
#+end_src
#+begin_src latex
  Inside \texttt{AddChild}, we set the parent link. The child is either
  assigned to the \texttt{Child} link or after scanning the sibling
  list.
#+end_src
#+begin_src go <<Methods, Ch.~\ref{ch:new}>>=
  ...
  func (n *Node) AddChild(v *Node) {
	  ...
	  if n.Child == nil {
		  ...
		  //<<Scan sibling list, Ch.~\ref{ch:new}>>
	  }
  }
#+end_src
#+begin_src latex
...
#+end_src
#+begin_src go <<Scan sibling list, Ch.~\ref{ch:new}>>=
  w := n.Child
  for w.Sib != nil {
	  w = w.Sib
  }
