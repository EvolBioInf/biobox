...
#+end_src
#+begin_src latex
  The help option, \ty{-h}, is always implied. We declare two other
  options. The input lines to \ty{rep2plot} might be very long, so the
  user can increase the maximum token size read. We also declare the
  version, \ty{-v}.
#+end_src
#+begin_src go <<Delcare options, Ch.~\ref{ch:r2p}>>=
  t := 128 * 1026
  var optT = flag.Int("t", t, "maximum scan token size")
  var optV = flag.Bool("v", false, "print program version " +
	  "and other information")
#+end_src
#+begin_src latex
  We import \ty{flag} and \ty{bufio}.
#+end_src
#+begin_src go <<Imports, Ch.~\ref{ch:r2p}>>=
  "flag"
  "bufio"
...
#+begin_src go <<Parse input files, Ch.~\ref{ch:r2p}>>=
  files := flag.Args()
  clio.ParseFiles(files, scan, *optT)
#+end_src
#+begin_src latex
  Inside scan we retrieve the maximum scan token size, create the
  scanner, scan the file, check the scanner for error, and print the
  segments.
#+end_src
#+begin_src go <<Functions, Ch.~\ref{ch:r2p}>>=
  func scan(r io.Reader, args ...interface{}) {
	  t := args[0].(int)
	  //<<Create scanner, Ch.~\ref{ch:r2p}>>
	  //<<Scan file, Ch.~\ref{ch:r2p}>>
	  //<<Check scanner, Ch.~\ref{ch:r2p}>>
...
#+end_src
#+begin_src latex
  The scanner has the maximum scan token size set by the user.
#+end_src
#+begin_src go <<Create scanner, Ch.~\ref{ch:r2p}>>=
  b := make([]byte, 2)
...
#+end_src
#+begin_src latex
  We're done with the scanner and report any error it might have
  encountered.
#+end_src
#+begin_src go <<Check scanner, Ch.~\ref{ch:r2p}>>=
  if scanner.Err() != nil {
