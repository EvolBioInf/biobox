#+begin_src latex
  ...
  segments, which can then be rendered with \ty{pipePlot}
  (Chapter~\ref{ch:pp})) to give Figure~\ref{fig:ppAdh}.

  \begin{figure}
    \begin{center}
      \scalebox{0.5}{\includegraphics{ppAdh}}
    \end{center}
    \caption{Segment plot of the matches between the \emph{Adh} loci in
      \emph{Drosophila melanogaster} amd
      \emph{D. guanche}.\label{fig:ppAdh}
  \end{figure}
