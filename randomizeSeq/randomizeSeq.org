#+begin_src latex
  ...
  We often compare the properties of some sequence with those of its
  shuffled version. The program \texttt{randomizeSeq} carries out this
  shuffling. Its outline contains hooks for imports, variable,
...
#+begin_src latex
  ...
  arguments the names of the files to be parsed, a function applied to
  each file, and its argument, a pointer to the random number generator.
#+end_src
...
#+begin_src latex
  ...
  and print a shuffled version of each \texttt{Sequence} in the
  input. We append \texttt{SHUFFLED} to the header to notify the user
...
#+end_src
#+begin_src latex
  This concludes \texttt{randomizeSeq}, testing time.
  \section*{Testing}
...
#+begin_src latex
  ...
  its result, \texttt{test.fasta} becomes \texttt{shuf.fasta}. So we can
  compare what we get when we shuffle \texttt{test.fasta} with what we
  want.
#+end_src
