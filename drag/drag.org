#+begin_src latex
  ...
  \begin{figure}
    ...
    \caption{Genealogy with genetic ancestry traced for all
      individuals (\textbf{A}) or just one (\textbf{B}).}\label{fig:dgn}
  \end{figure}
...
#+end_src
#+begin_src latex
  Apart from the version, we declare six options: number of generations
  (\ty{-g}), number of individuals (\ty{-n}), trace the ancestry of a
  set of individuals (\ty{-t}), a scaling factor for the plot (\ty{-f}),
  just ancestor statistics instead of the graph (\ty{-a}), and the seed
...
#+begin_src latex
  ...
  and may lie on a path to be drawn. It may also be a universal ancestor
  or a non-ancestor.
#+end_src
...
#+begin_src latex
  ...
  it, pick ancestors, and trace individuals.
#+end_src
...
#+begin_src latex
  ...
  descendants to that of its ancestor. Then we determine ancestor status
  of all individuals.
#+end_src
...
#+end_src
#+begin_src latex
  To trace individuals, we initialize the paths formed by their lines of
  descent and then complete them.
#+end_src
...
#+end_src
#+begin_src latex
  There are two edges per individual, one for each gene.
#+end_src
...
#+end_src
#+begin_src latex
  We construct six tests. The first is without any options apart from
  the seed for the random number generator. The second sets the number
  of generations, the third the number of individuals, the fours traces
  individuals 3 and 4, the fifth traces all individuals, and the sixth
  prints the ancestor stats.
#+end_src
