#+begin_src latex
  ...
  sequences in the input. Positions are one-based and start/end
  coordinates are inclusive. It is also possible to join the regions.
...
#+end_src
#+begin_src latex
  We continue parsing the options by reading the regions supplied either
  on the command line or in a file into a slice.
#+end_src
#+begin_src go <<Parse options, Ch.~\ref{ch:cut}>>=
  var regions []region
  if *optR != "" {
	  ...
	  fmt.Fprintf(os.Stderr,
		  "Please provide a region to cut either via -r or -f.")
	  os.Exit(0)
  }
#+end_src
#+begin_src latex
...
#+end_src
#+begin_src latex
  An individual region is a structure that holds a pair of start and end
  positions.
#+end_src
...
#+end_src
#+begin_src latex
  On the command line the regions are separated by commas, and the
  positions by hyphens.
#+end_src
...
#+end_src
#+begin_src latex
  If we cant open the input file, we abort.
#+end_src
...
#+end_src
#+begin_src latex
  The file of regions is scanned using a \texttt{Scanner}.
#+end_src
...
#+end_src
#+begin_src latex
  When joining, we store the bits of sequence in a byte slice and
  prepare a header that documents which regions were stuck together.
#+end_src
...
#+end_src
#+begin_src latex
  Finally, we join the two regions.
#+end_src
#+begin_src go <<Testing, Ch.~\ref{ch:cut}>>=
