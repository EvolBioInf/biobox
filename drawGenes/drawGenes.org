#+begin_src latex
  ...
  in Figure~\ref{fig:bw}B and plotted in Figure~\ref{fig:bw}C. The
  program \ty{drawGenes} takes data like that in Figure~\ref{fig:bw}A
  and converts it the x/y coordinates in Figure~\ref{fig:bw}B, ready
  for plotting.
