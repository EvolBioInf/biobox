...
#+end_src
#+begin_src latex
  The regular expression identifying the sequence is passed via
  \texttt{-s}. In response to \texttt{-c}, the program prints the
...
#+begin_src go <<Variables, Ch.~\ref{ch:get}>>=
  var optS = flag.String("s", ".", "sequence identified by regex")
...
#+end_src
#+begin_src latex
  After parsing the options, we test for \texttt{-v}.
#+end_src
...
#+begin_src go <<Testing, Ch.~\ref{ch:get}>>=
  cmd := exec.Command("getSeq", "-s", "Seq1", "test.fasta")
  o, err := cmd.Output()
  if err != nil {
...
#+end_src
#+begin_src go <<Testing, Ch.~\ref{ch:get}>>=
  cmd = exec.Command("getSeq", "-s", "1$", "test.fasta")
  o, err = cmd.Output()
  if err != nil {
	  t.Errorf("couldn't run %q\n", cmd)
  }
...
#+end_src
#+begin_src go <<Testing, Ch.~\ref{ch:get}>>=
  cmd = exec.Command("getSeq", "-s", "[123]$", "test.fasta")
  o, err = cmd.Output()
  if err != nil {
	  t.Errorf("couldn't run %q\n", cmd)
  }
...
#+end_src
#+begin_src go <<Testing, Ch.~\ref{ch:get}>>=
  cmd = exec.Command("getSeq", "-s", "[123]$", "-c",
	  "test.fasta")
  o, err = cmd.Output()
  if err != nil {
	  t.Errof("couldn't run %q\n", cmd)
  }
