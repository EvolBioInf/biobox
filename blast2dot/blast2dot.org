...
#+begin_src go <<Set usage, Ch.~\ref{ch:b2d}>>=
  ...
  p := "Convert BLAST output to dot code " +
	  "for plotting with neato or circo (GraphViz)."
  e := "blast2dot foo.bl > foo.dot"
  clio.Usage(u, p, e)
#+end_src
#+begin_src latex
  We import \ty{clio}.
...
#+begin_src latex
  ...
  include singletons in the output, but the user can chose to do
  so. (S)he can also ask for the version.
#+end_src
...
#+begin_src latex
  ...
  input files. These are parsed with the function \ty{scan}, which
  takes as argument the \ty{-s} option.
#+end_src
...
#+end_src
#+begin_src latex
  In \ty{scan} we retrieve the \ty{-s} option, read the accessions, and
...
#+begin_src go <<Write header, Ch.~\ref{ch:b2d}>>=
  ...
  fmt.Println("# Render: neato|circo foo.dot")
  fmt.Println("graph G {")
#+end_src
#+begin_src latex
  We import \ty{fmt}.
...
#+end_src
#+begin_src latex
  The members of gene families are plotted in nodes filled in light
  salmon. Then we go through the match matrix and write the
  query/subject pairs.
#+end_src
#+begin_src go <<Write gene families, Ch.~\ref{ch:b2d}>>=
  fmt.Println("node [style=filled, color=lightsalmon]")
  for i, v := range mm {
	  for j, _ := range v {
		  if i != j && mm[i][j] {
...
#+begin_src latex
  ...
  family. Their nodes are filled light gray.
#+end_src
#+begin_src go <<Write singletons, Ch.~\ref{ch:b2d}>>=
  fmt.Println("node [style=filled, color=lightgray]")
...
#+end_src
#+begin_src latex
  We construct two tests, one without singletons, one with.
#+end_src
#+begin_src go <<Construct tests, Ch.~\ref{ch:b2d}>>=
  f := "test.bl"
  cmd := exec.Command("./blast2dot", f)
  tests = append(tests, cmd)
  cmd = exec.Command("./blast2dot", "-s", f)
  tests = append(tests, cmd)
#+end_src
#+begin_src latex
