...
#+begin_src latex
  ...
  options, and compute the random variable.
#+end_src
...
#+end_src
#+begin_src latex
  The user can set the mean number of events, \ty{-m}, and the seed for
  the random number generator, \ty{-s}, which is a long integer. (S)he
  can also ask for the version.
#+end_src
