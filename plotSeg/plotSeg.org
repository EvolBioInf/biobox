...
#+end_src
#+begin_src latex
  We write the plot as an untitled black line plot with ranges. We
  switch the tic marks from the x axis at the bottom to the top and we
  reverse the y axis. We label the top x axis and rotate the y axis to
  read from top to bottom. Just before drawing the actual plot, we enter
  the \ty{gnuplot} code submitted by the user.
#+end_src
#+begin_src go <<Write plot, Ch.~\ref{ch:ps}>>=
  ...
  fmt.Fprintf(w, "unset xtics; set x2tics mirror\n")
  fmt.Fprintf(w, "set yrange [*:*] reverse\n")
