...
#+begin_src go <<Set usage, Ch.~\ref{ch:ps}>>=
  ...
  e := "a) plotSeg foo.dat\n" +
	  "\t b) plotSeg -s foo.r foo.dat; " +
	  "R -f foo.r < foo.dat > /dev/null"
  clio.Usage(u, p, e)
#+end_src
#+begin_src latex
  We import \ty{clio}.
...
#+end_src
#+begin_src latex
  Instead of showing the plot in a window, the user can opt for a
  postscript file and set the width and height of the postscript plot in
  cm. The user can also ask for the R script instead of any plot.
#+end_src
...
#+begin_src latex
  ...
  and the graphics window. This is x11 by default, but if we are running
  on the macOS command line, we switch to quartz. We also collect the
  names of the files for postscript and R output.
#+end_src
...
#+begin_src go <<Parse input files, Ch.~\ref{ch:ps}>>=
  ...
  err = os.Remove(rscr)
  if err != nil { log.Fatalf("can't delete %q", rscr) }
#+end_src
#+begin_src latex
...
#+end_src
#+begin_src latex
  If the user requested the R script, we write to that, otherwise we
...
#+end_src
#+begin_src latex
  We construct the template in four steps. Construct the header,
...
#+begin_src go <<Deal with axis labels, Ch.~\ref{ch:ps}>>=
  {{- if .Xlab -}}
  p <- p + xlab("{{.Xlab}}"){{printf "\n"}}
  {{- else -}}
  p <- p + xlab(NULL){{printf "\n"}}
  {{- end -}}
  {{- if .Ylab -}}
  p <- p + ylab("{{.Ylab}}"){{printf "\n"}}
  {{- else -}}
  p <- p + ylab(NULL){{printf "\n"}}
  {{- end -}}
#+end_src
#+begin_src latex
...
#+begin_src go <<Deal with axis ranges, Ch.~\ref{ch:ps}>>=
  {{- if and .Xmin .Xmax -}}
  p <- p + xlim({{.Xmin}}, {{.Xmax}}){{printf "\n"}}
  {{- end -}}
  {{- if and .Ymin .Ymax -}}
  p <- p + ylim({{.Ymin}}, {{.Ymax}}){{printf "\n"}}
  {{- end -}}
#+end_src
#+begin_src latex
  We set postscript, or window with the plot height \&
  width. Then we draw the plot and close the device.
#+end_src
#+begin_src r <<Deal with device options, Ch.~\ref{ch:ps}>>=
  {{- if .Ps -}}
  //<<Set postscript file, Ch.~\ref{ch:ps}>>
  {{- else -}}
  //<<Set window, Ch.~\ref{ch:ps}>>
  {{- end -}}
...
#+begin_src latex
  We set the window.
#+end_src
#+begin_src r <<Set window, Ch.~\ref{ch:ps}>>=
  {{.Win}}(
#+end_src
#+begin_src latex
...
#+end_src
#+begin_src latex
  If we spawned a window, we make sure the script exits once the
...
#+begin_src go <<Close device, Ch.~\ref{ch:ps}>>=
  ...
  while(names(dev.cur()) != 'null device')
      Sys.sleep(0.01){{printf "\n"}}
  {{- else -}}
  dev.off(){{printf "\n"}}
  {{- end -}}
#+end_src
#+begin_src latex
  The R script is written, so we run it.
#+end_src
#+begin_src go <<Construct R command, Ch.~\ref{ch:ps}>>=
  rcmd = exec.Command("R", "-f", rscr)
#+end_src
#+begin_src latex
...
#+end_src
#+begin_src latex
  Piping the data into the R command is wrapped in a goroutine. The
  routine signals it is finished via a channel.
#+end_src
...
#+begin_src latex
  ...
  loop. Each test is written to the same unique R file, which we remove
...
#+begin_src go <<Test axis labels, Ch.~\ref{ch:ps}>>=
  f := "test.dat"
  te := exec.Command("./plotSeg", "-s", r, f)
  tests = append(tests, te)
  te = exec.Command("./plotSeg", "-s", r, "-x", "x", f)
  tests = append(tests, te)
  te = exec.Command("./plotSeg", "-s", r, "-y", "y", f)
  tests = append(tests, te)
  te = exec.Command("./plotSeg", "-s", r, "-x", "x",
	  "-y", "y", f)
  tests = append(tests, te)
#+end_src
#+begin_src latex
...
#+begin_src go <<Test axis ranges, Ch.~\ref{ch:ps}>>=
  te = exec.Command("./plotSeg", "-s", r, "-X", "100:500", f)
  tests = append(tests, te)
  te = exec.Command("./plotSeg", "-s", r, "-Y", "100:500", f)
  tests = append(tests, te)
  te = exec.Command("./plotSeg", "-s", r, "-X", "100:500",
	  "-Y", "100:500", f)
  tests = append(tests, te)
#+end_src
#+begin_src latex
...
#+begin_src go <<Test plot size, Ch.~\ref{ch:ps}>>=
  te = exec.Command("./plotSeg", "-s", r, "-w", "5", f)
  tests = append(tests, te)
  te = exec.Command("./plotSeg", "-s", r, "-H", "6", f)
  tests = append(tests, te)
  te = exec.Command("./plotSeg", "-s", r, "-w", "5",
	  "-H", "6", f)
  tests = append(tests, te)
#+end_src
#+begin_src latex
...
#+begin_src go <<Run test, Ch.~\ref{ch:ps}>>=
  ...
  if err != nil { log.Fatalf("can't read %q", r) }
...
#+begin_src latex
  ...
  \ty{results/r1.r}, \ty{results/r2.r}, and so on. On darwin systems,
