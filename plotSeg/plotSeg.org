...
#+begin_src go <<Declare device options, Ch.~\ref{ch:ps}>>=
  ...
  var optD = flag.String("d", "x,y", "dimensions of postscript " +
	  "plot in inch or cm")
...
#+end_src
#+begin_src latex
  Inside \ty{scan}, we retrieve the options, read the segments, and
  construct the output stream. We use a Go routine to write the
  \ty{gnuplot} code to that output stream, which we also close inside
  that routine. After the Go routine, we run \ty{gnuplot}, unless the
  user opted for the script instead.
#+end_src
...
#+end_src
#+begin_src latex
  The \ty{gnuplot} code describes the terminal, the plot, and the
  segments.
#+end_src
...
#+end_src
#+begin_src latex
  We write the plot as an untitled black line plot with ranges.
#+end_src
#+begin_src go <<Write plot, Ch.~\ref{ch:ps}>>=
  ...
  fmt.Fprintf(w, "plot[%s][%s] \"-\" t '' w l lc \"black\"\n",
	  xr, yr)
#+end_src
#+begin_src latex
