...
#+end_src
#+begin_export latex
The one option we haven't collected yet is the terminal. This is wxt
by default, but if we are running on darwin under macOS, the default
is qt.
#+end_export
#+begin_src go <<Collect option values, Ch.~\ref{ch:ps}>>=
  opts.Win = *optT
...
#+end_src
#+begin_src latex
  We run \ty{gnuplot} and check its error.
#+end_src
