...
#+begin_src go <<Set usage, Ch.~\ref{ch:ps}>>=
  ...
  e := "mum2plot eco_x_y.mum | plotSeg -S 7.5cm,7.5cm"
  clio.Usage(u, p, e)
#+end_src
#+begin_src latex
  We import \ty{clio}.
...
#+begin_src go <<Open script file, Ch.~\ref{ch:ps}>>=
  ...
  if err != nil {
	  log.Fatal("can't open gnuplot script")
  }
  gscr = script.Name()
#+end_src
#+begin_src latex
...
#+end_src
#+begin_src latex
  We write the script by writing to the terminal first. Then we write
  the axes and the actual plot.
#+end_src
...
#+end_src
#+begin_src latex
  We set the axis labels the user supplied.
#+end_src
...
#+end_src
#+begin_src latex
  We run the command and signal we're done before deleting the script.
#+end_src
