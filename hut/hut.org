...
#+begin_src latex
  ...
  zero or a one, an underscore, the character encoded, and a slash. In
...
#+begin_src latex
  ...
  length of its label minus the length of the code prefix, 4. The result
  gets added to the current weight and returned.
#+end_src
#+begin_src go <<Functions, Ch.~\ref{ch:hut}>>=
  func sumLeafWeights(v *nwk.Node, w float64) float64 {
	  if v == nil { return w }
	  if v.Child == nil {
		  cl := float64(len(v.Label) - 4)
		  w += v.Length * cl
	  }
	  w = sumLeafWeights(v.Child, w)
	  w = sumLeafWeights(v.Sib, w)
	  return w
  }
#+end_src
#+begin_src latex
