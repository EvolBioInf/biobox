#+begin_src latex
  ...
  Characters typically take up one byte of computer memory. This is true
  even for texts over alphabets much smaller than the ASCII code, or
  where characters frequencies vary
  ...
  the character to be encoded and its
  frequency~\cite[p. 385ff]{cor01:int}. The frequencies are interpreted
  as node weights, and the two lightest leaves are merged into a parent
  whose weight is the sum of its children's weights. This procedure is
  repeated until the last two nodes are merged into the root.
  ...
  applied to the actual genome sequence, it returns the tree in
  Figure~\ref{fig:hut}B, where branch lengths are proportional to node
  weights.
...
#+end_src
#+begin_src latex
  There is only one option to declare, the version, \ty{-v}.
#+end_src
...
#+begin_src latex
  ...
  files, which we parse using the function \ty{scan}.
#+end_src
...
#+end_src
#+begin_src latex
  Inside \ty{scan} we iterate over the sequences contained in the
  file. For each sequence we count the characters, construct a tree, and
  print it.
#+end_src
...
#+end_src
#+begin_src latex
  To count the characters, we reset all counts to zero and iterate over
  the sequence data. We are going to build a binary tree from these
  characters. This only works if there are at least two characters to
  construct a tree from. So we ensure that's the case.
#+end_src
#+begin_src go <<Count characters, Ch.~\ref{ch:hut}>>=
  for i, _ := range counts { counts[i] = 0 }
...
#+begin_src latex
  ...
  that label the leaves in a separate map referenced by their ID.
#+end_src
...
#+begin_src latex
  ...
  to find the rest of the code in right-to-left orientation. The code
  thus collected is still written right-to-left. So we reverse it before
  storing it as part of the leaf label.
#+end_src
#+begin_src go <<Functions, Ch.~\ref{ch:hut}>>=
...
#+end_src
#+begin_src latex
  We label a leaf for character \ty{c} using the pattern
  \begin{verbatim}
  [0|1]_c/code
  \end{verbatim}
#+end_src
#+begin_src go <<Store leaf label, Ch.~\ref{ch:hut}>>=
  n.Label = fmt.Sprintf("\"%s_%c/%s\"", n.Label, labels[n.Id], code)
#+end_src
#+begin_src latex
  We import \ty{fmt}.
#+end_src
...
#+end_src
#+begin_src latex
  We print the tree by printing its root.
#+end_src
...
#+end_src
#+begin_src latex
  We construct one test and then run it.
#+end_src
#+begin_src go <<Testing, Ch.~\ref{ch:hut}>>=
  //<<Construct test, Ch.~\ref{ch:hut}>>
...
#+begin_src latex
  ...
  \ty{test.fasta}.
#+end_src
#+begin_src go <<Construct test, Ch.~\ref{ch:hut}>>=
  test := exec.Command("./hut", "test.fasta")
#+end_src
#+begin_src latex
  We import \ty{exec}.
#+end_src
