...
#+begin_src latex
  ...
  opt for points or lines only instead. She can also opt to write the
...
#+begin_src latex
  ...
  plot it produces. Among the graphical devices supported by
  \ty{gnuplot}, we offer only encapsulated postscript as an alternative
  to the default window.
#+end_src
#+begin_src go <<Declare device options, Ch.~\ref{ch:pl}>>=
  var optP = flag.String("p", "", "encapsulated postscript file")
  var optSS = flag.String("S", "x,y", "size of postscript plot in inch or, if stated, cm")
#+end_src
#+begin_src latex
  We parse the options and respond to \ty{-v} first, as this might
  terminate the program. Then we collect the option values.
#+end_src
...
#+begin_src latex
  ...
  analysis, \ty{-x}, \ty{-y}, \ty{-X}, \ty{-Y}, \ty{-S}, \ty{-d},
  \ty{-P}, \ty{-L}, \ty{-l}, \ty{-s}, and \ty{-p}.
#+end_src
...
#+begin_src latex
  ...
  assertion, read the data, and extract the categories. If we're asked
  to write the script, we exit afterwards. Otherwise, we construct the
  command that runs the script and run it. Unless the user asked for the
  script, we delete it at the end.
#+end_src
...
#+end_src
#+begin_src latex
  We run construct the \ty{gnuplot} command.
#+end_src
...
#+begin_src latex
  ...
  output to a temporary \ty{gnuplot} script, so that we can compare what we get to
...
#+end_src
#+begin_src latex
  We set the plot size, which onlyh works with postscript output.
#+end_src
