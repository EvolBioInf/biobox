...
#+end_src
#+begin_src latex
  If we are running an x11 window, we check every onehundredth second
...
#+end_src
#+begin_src go <<Delete data, Ch.~\ref{ch:pl}>>=
  err = os.Remove(args.File)
  if err != nil {
	  log.Fatalf("can't remove %q", args.File)
  }
#+end_src
#+begin_src latex
  We run R in ``vanilla'' mode and send the standard output stream to
  null.
#+end_src
#+begin_src go <<Construct R command, Ch.~\ref{ch:pl}>>=
  cmd := exec.Command("R", "--vanilla", "-f", script.Name(),
	  "> /dev/null")
#+end_src
#+begin_src latex
  We import \ty{exec}.
