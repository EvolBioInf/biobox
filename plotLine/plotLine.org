...
#+begin_src latex
  ...
  analysis, \ty{-x}, \ty{-y}, \ty{-H}, \ty{-w}, \ty{-d}, \ty{-D}, and
  \ty{-p}. R takes plot height and width in inches, but \ty{plotLine}
  works in cm. So we convert from cm to inch by dividing by 2.54.
#+end_src
...
#+end_src
#+begin_src latex
The script is written either to a unique file. 
#+end_src
...
#+end_src
#+begin_src latex
  We write the script using a template.
#+end_src
...
#+begin_src go <<Testing, Ch.~\ref{ch:pl}>>=
  ...
  err = os.Remove(ps)
  if err != nil { log.Fatalf("can't delete %q", ps) }
#+end_src
#+begin_src latex
...
#+begin_src go <<Test ranges, Ch.~\ref{ch:pl}>>=
  ...
  test = exec.Command("./plotLine", "-p", ps,
	  "-Y", "0.2:100", "test3.dat")
  tests = append(tests, test)
  test = exec.Command("./plotLine", "-p", ps, "-X", "0.1:10",
	  "-Y", "0.2:100", "test3.dat")
  tests = append(tests, test)
#+end_src
#+begin_src latex
