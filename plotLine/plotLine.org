...
#+begin_src go <<Set usage, Ch.~\ref{ch:pl}>>=
  ...
  e := "plotLine -S 7.5cm,4.75cm foo.dat"
  clio.Usage(u, p, e)
#+end_src
#+begin_src latex
  We import \ty{clio}.
...
#+begin_src go <<Declare axes options, Ch~\ref{ch:pl}>>=
  ...
  var optU = flag.String("u", "", "unset axis (x|y|xy)")
#+end_src
#+begin_src latex
...
#+begin_src go <<Declare plot type options, Ch.~\ref{ch:pl}>>=
  ...
  var optLL = flag.Bool("L", false, "lines and points")
...
#+begin_src latex
  ...
  case, the user can also set the plot dimensions.
#+end_src
#+begin_src go <<Declare device options, Ch.~\ref{ch:pl}>>=
  var optP = flag.String("p", "", "encapsulated postscript file")
  var optD = flag.String("d", "x,y",
	  "dimensions of postscript plot in inch or cm")
#+end_src
#+begin_src latex
...
#+begin_src latex
  ...
  \ty{-u}, \ty{-d}, \ty{-P}, \ty{-L}, \ty{-l}, \ty{-s}, and \ty{-p}.
#+end_src
...
#+begin_src latex
  ...
  \ty{gnuplot} code to this output stream inside a Go routine and close
  it again. Outside of the Go routine we run \ty{gnuplot}, unless the
...
#+begin_src go <<Test unsetting axes, Ch.~\ref{ch:pl}>>=
  test = exec.Command("./plotLine", "-u", "x", "test3.dat")
  tests = append(tests, test)
  test = exec.Command("./plotLine", "-u", "y", "test3.dat")
  tests = append(tests, test)
  test = exec.Command("./plotLine", "-u", "xy", "test3.dat")
  tests = append(tests, test)
#+end_src
#+begin_src latex
