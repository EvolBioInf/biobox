...
#+begin_src go <<Set usage, Ch.~\ref{ch:pl}>>=
  ...
  e := "a) plotLine foo.dat\n" +
	  "\t b) plotLine -s foo.r foo.dat; R -f foo.r < foo.dat > /dev/null"
  clio.Usage(u, p, e)
#+end_src
#+begin_src latex
  We import \ty{clio}.
...
#+begin_src latex
  ...
  assertion, read and write the data, and construct the R script. If
  we're asked to write the R script, we exit afterwards. Otherwise, we
  construct the R command that runs the script and run it. Then we
  delete the R script and the data.
#+end_src
...
#+end_src
#+begin_src latex
  We open a file, store its name as one of the arguments, and write the
  data to it.
#+end_src
#+begin_src go <<Write data, Ch.~\ref{ch:pl}>>=
  file, err := ioutil.TempFile("", "tmp_*.dat")
  if err != nil {
	  log.Fatalf("can't create data file")
  }
  defer file.Close()
  args.File = file.Name()
  for _, d := range data {
...
#+end_src
#+begin_src latex
  We add the field \ty{File} to \ty{Args}.
#+end_src
#+begin_src go <<\ty{Args} fields, Ch.~\ref{ch:pl}>>=
  File string
#+end_src
...
#+end_src
#+begin_src latex
  We open a file to write the script to.
#+end_src
...
#+begin_src latex
  ...
  data from the file we wrote them to.
#+end_src
...
#+begin_src r <<Deal with axes options, Ch.~\ref{ch:pl}>>=
  {{- if .Xlab -}}
  plot <- plot + xlab("{{.Xlab}}"){{printf "\n"}}
  {{- else -}}
  plot <- plot + xlab(NULL){{printf "\n"}}
  {{- end -}}
  {{- if .Ylab -}}
  plot <- plot + ylab("{{.Ylab}}"){{printf "\n"}}
  {{- else -}}
  plot <- plot + ylab(NULL){{printf "\n"}}
  {{- end -}}
#+end_src
#+begin_src latex
...
#+begin_src r <<Scale x-axis, Ch.~\ref{ch:pl}>>=
  ...
  {{- else if .Xrange -}}
  plot <- plot + xlim({{.Xmin}}, {{.Xmax}}){{printf "\n"}}
  {{- end -}}
#+end_src
#+begin_src latex
...
#+begin_src r <<Scale y-axis, Ch.~\ref{ch:pl}>>=
  ...
  {{- else if .Yrange -}}
  plot <- plot + ylim({{.Ymin}}, {{.Ymax}}){{printf "\n"}}
  {{- end}}
#+end_src
#+begin_src latex
...
#+end_src
#+begin_src latex
  If we are running an x11 window, we check every one hundredth second
  whether it's still active. If is isn't active any more, we end the
  script. This crutch simulates the expected behavior that \ty{plotLine}
  returns when the graphics window is closed. If instead of running an
  x11 window we print postscript output, we switch the device off
  afterwards.
#+end_src
...
#+begin_src go <<Construct R command, Ch.~\ref{ch:pl}>>=
  cmd := exec.Command("R", "-f", script.Name())
#+end_src
#+begin_src latex
  We import \ty{exec}.
#+end_src
...
#+end_src
#+begin_src latex
  We run the command and check for errors.
#+end_src
