...
#+begin_src latex
  ...
  the options and the input files. Then we calculate the overlap graph
  and draw it.
#+end_src
...
#+end_src
#+begin_src latex
  If $k$ is less than the length of \ty{b}, we increment it so $k$ can
  serve as right hand border. If, on the other hand, $k$ is greater than
  the length of \ty{b}, we bail with message.
#+end_src
...
#+begin_src go <<Find overlap, Ch.~\ref{ch:olga}>>=
  for true {
	  s = bytes.Index(a[s:], b[0:k])
	  if s == -1 { return 0 }
	  if bytes.HasPrefix(b, a[s:]) {
		  return len(a) - s
	  }
	  s++
  }
...
#+begin_src latex
  ...
  rendered. Then we open the actual graph, which is a directed graph.
#+end_src
...
#+end_src
#+begin_src latex
  We print all non-zero edges labeled by the overlap.
#+end_src
