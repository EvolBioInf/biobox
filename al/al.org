#+begin_src latex
  ...
  \end{eqnarray}
  For local alignment, the maximization for $V(i,j)$ includes zero:
  \[
...
#+begin_src latex
  ...
  query and subject files, and finally the score matrix.
#+end_src
...
#+end_src
#+begin_src latex
  When analyzing the input files, we make sure that the user has
...
#+begin_src latex
  ...
  the query. Regardless of the alignment type computed, thy all take as
  input a dynamic programming matrix of the same shape and two byte
  slices for storing the aligned sequences during trace back.
#+end_src
...
#+end_src
#+begin_src latex
  There are three types of alignment to choose from, global, overlap,
  and local (Figure~\ref{fig:al}).
#+end_src
...
#+begin_src latex
  ...
  first sequence is implicitly written along the x-dimension, the second
  sequence along the y-dimension (Figure~\ref{fig:dpm}). Once the
  alignment matrix is filled, we have determined the score.
#+end_src
...
#+begin_src latex
  ...
  equation~(\ref{eq:dp}). In addition, it contains a switch to record
  that a cell was visited during trace back. We will use this later
  during the trace back for local alignment.
#+end_src
...
#+end_src
#+begin_src latex
  The matrix is filled by visiting each entry.
#+end_src
#+begin_src go <<Fill global, Ch.~\ref{ch:al}>>=
  ...
  for i := 1; i <= m; i++ {
	  for j := 1; j <= n; j++ {
		  //<<Determine global entry, Ch.~\ref{ch:al}>>
	  }
  }
#+end_src
#+begin_src latex
  We determine a matrix entry by copying the recursions of the
  Introduction.
#+end_src
#+begin_src go <<Determine global entry, Ch.~\ref{ch:al}>>=
  p[i][j].e = math.Max(p[i][j-1].e, p[i][j-1].v + gapO) + gapE
  p[i][j].f = math.Max(p[i-1][j].f, p[i-1][j].v + gapO) + gapE
  p[i][j].g = p[i-1][j-1].v + mat.Score(d1[j-1], d2[i-1])
...
seq1 := fasta.NewSequence(s1.Header(), a1)
seq2 := fasta.NewSequence(s2.Header(), a2)
seq1.Reverse()
seq2.Reverse()
al := util.NewAlignment(seq1, seq2, mat,
	n-1, m-1, start1, start2, score)
al.SetLineLength(ll)
fmt.Println(al)
#+end_src
