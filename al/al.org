...
#+begin_src latex
  ...
  independent of the type of alignment we are dealing with, so we give
  it its own chunk.
#+end_src
...
#+end_src
#+begin_src latex
  To output an alignment, we create two sequences from the aligned query
  and subject, reverse the alignments, and create a dedicated alignment
  data structure. On this we set the line length entered by the user and
  then print it.
#+end_src
