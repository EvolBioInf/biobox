#+begin_src latex
    ...
    tree with labeled nodes. The corresponding text version
    (Figure~\ref{fig:kt}C) is in Newick
  ...
  \end{center}
	...
	but with labeled nodes (\textbf{B}), and in text format
...
#+begin_src go <<Draw tree, Ch.~\ref{ch:dkt}>>=
  ...
  if *optT {
	  ...
	  fmt.Println(writeLatex(tree, *optL, *optX))
	  if *optW != "" {
...
#+end_src
#+begin_src latex
  Failure links are red arks $v\rightarrow\mbox{fail(v)}$. There is a
