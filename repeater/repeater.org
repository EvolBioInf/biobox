#+begin_src latex
  ...
  orientation. However, getting from the array to the tree is easier if
  ...
  a maximal repeat, unless, of course, it's the root.
...
#+begin_src latex
  ...
  by subtracting it from the end. We also halve the sequence identifier.
#+end_src
...
#+begin_src latex
  ...
  position in that sequence. The start is either zero of follows the end
  of its predecessor.
#+end_src
