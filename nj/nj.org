...
#+begin_src latex
  ...
  the intermediate distance matrices, \ty{-m}.
#+end_src
...
#+begin_src latex
  ...
  turn takes the \ty{-m} option as argument.
#+end_src
...
#+end_src
#+begin_src latex
  Inside \ty{scan}, we retrieve \ty{-m} and iterate over the distance
...
#+begin_src go <<Process distance matrix, Ch.~\ref{ch:nj}>>=
  ...
  var root *newick.Node
  //<<Calculate tree, Ch.~\ref{ch:nj}>>
  fmt.Println(root)
#+end_src
#+begin_src latex
  We import \ty{newick} and \ty{fmt}.
#+end_src
#+begin_src go <<Imports, Ch.~\ref{ch:nj}>>=
...
#+begin_src latex
  ...
  sense of the printed matrices. So in a last step, we remove these
  labels again, as phylogenies only have leaf labels.
#+end_src
...
#+end_src
#+begin_src go <<Functions, Ch.~\ref{ch:nj}>>=
  func resetLabels(v *newick.Node) {
	  if v == nil { return }
	  resetLabels(v.Child)
	  resetLabels(v.Sib)
	  if v.Child != nil {
