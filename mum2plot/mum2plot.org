#+begin_src latex
  ...
  Table~\ref{fig:mum}B, which can be plotted with \ty{pipePlot} to give
  Figure~\ref{fig:mum}.

  \begin{figure}
    \begin{center}
      \scalebox{0.3}{\rotatebox{-90}{\includegraphics{mummer.ps}}}
    \end{center}
    ...
    \ty{mum2plot | pipePlot}.}\label{fig:mum}
  \end{figure}
...
#+begin_src go <<Set usage, Ch.~\ref{ch:m2p}>>=
  ...
  e := "mummer -b -c s1.fasta s2.fasta | mum2plot | pipePlot"
  clio.Usage(u, p, e)
#+end_src
#+begin_src latex
  We import \ty{clio}.
...
#+end_src
#+begin_src latex
  We compute the end coordinates depending on the list we're reading.
#+end_src
