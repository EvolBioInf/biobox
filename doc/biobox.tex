...
\input{bwt}
\chapter{Program \ty{clac}: Clade Counter}\label{ch:clac}
\input{clac}
\chapter{Program \ty{coat}: Coalescence Times}\label{ch:coa}
\input{coat}
...
\input{dnaDist}
\chapter{Program \ty{drag}: Draw Genealogies}\label{ch:dgn}
\input{drag}
\chapter{Program \ty{drawf}: Draw Wright-Fisher
...
\chapter{Program \ty{fasta2tab}: Convert FASTA to Tabular
  Format}\label{ch:f2t}
\input{fasta2tab}
\chapter{Program \ty{geco}: Explore the Genetic Code}\label{ch:gc}
\input{geco}
...
\input{histogram}
\chapter{Program \ty{huff}: Huffman Encoding}\label{ch:huf}
\input{huff}
\chapter{Program \ty{hut}: Calculate Huffman tree}\label{ch:hut}
\input{hut}
\chapter{Program \ty{kerror}: $k$-Error Alignment}\label{ch:ke}
\input{kerror}
\chapter{Program \texttt{keyMat}: Matching Keywords}\label{ch:km}
\input{keyMat}
\chapter{Program \ty{maf}: Calculate Match Factors}\label{ch:ma}
\input{maf}
\chapter{Program \ty{midRoot}: Midpoint Rooting of
  Phylogenies}\label{ch:mr}
\input{midRoot}
\chapter{Program \ty{mtf}: Move to Front}\label{ch:mt}
\input{mtf}
...
\chapter{Program \texttt{naiveMatcher}: Match Pattern in
  Text}\label{ch:nm}
\input{naiveMatcher}
\chapter{Program \ty{num2char}: Convert Numbers to Characters}\label{ch:n2c}
\input{num2char}
...
\input{nj}
\chapter{Program \ty{olga}: Compute Overlap Graph}\label{ch:olga}
\input{olga}
\chapter{Program \texttt{pam}: Compute PAM Score
  Matrices}\label{ch:pam}
\input{pam}
\chapter{Program \ty{pickChildren}: Pick the Children of Coalescent
  Nodes}\label{ch:pc}
\input{pickChildren}
...
\input{plotTree}
\chapter{Program \ty{pps}: Print Polymorphic Sites}\label{ch:pp}
\input{pps}
...
\chapter{Program \ty{simOrf}: Simulate Open Reading
  Frames}\label{ch:so}
\input{simOrf}
\chapter{Program \ty{sops}: Sum-of-Pairs Score for Multiple Sequence
  Alignment}\label{ch:sops}
\input{sops}
\chapter{Program \ty{splitSeq}: Split a Sequence into Fragments}\label{ch:ss}
\input{splitSeq}
\chapter{Program \texttt{sw}: Sliding Window Analysis}\label{ch:sw}
\input{sw}
