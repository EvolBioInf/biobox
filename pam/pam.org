#+begin_src latex
  ...
  \]
  The relatedness matrix is symmetrical. The frequencies that go into
  its computation are contained in a file with 20 rows, one for each
  amino acid and its frequency:
  \begin{verbatim}
  A 0.087
  C 0.033       
  D 0.047
...
#+end_src
#+begin_src latex
  In the main function, we state the usage, declare and parse the
  options, and parse the input file.
#+end_src
...
#+end_src
#+begin_src latex 
  Having computed the new matrix, we print it. To line up the columns,
  we use a \texttt{tabwriter}. Once constructed, we fill it with the
  header and body of the table.
#+end_src
...
#+end_src
#+begin_src latex
  The \texttt{tabwriter} is used to write to a buffer. It is initialized
  to a minimal cell width of 1, zero-width tabs, and a single blank
  added for padding.
#+end_src
...
#+end_src
#+begin_src latex
  Each row of the table starts with an amino acid, followed by the
  numerical entries. When all the data are entered, the
