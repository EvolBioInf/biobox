...
#+end_src
#+begin_src latex
The user can also choose to draw the tree to a postscript
  ...
  height (\ty{-r}). The scale itself can also be set (\ty{-s}).
#+end_src
#+begin_src go <<Declare options, Ch.~\ref{ch:pt}>>=
...
#+begin_src latex
  ...
  want. The results we want are stored in the files \ty{r1.r},
  \ty{r2.r}, and so on.
#+end_src
