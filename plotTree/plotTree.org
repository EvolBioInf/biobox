#+begin_src latex
  ...
  in rooted layout (Figure~\ref{fig:phy}B) or unrooted layout
  (Figure~\ref{fig:phy}C). The user can also choose between
  postscript file or screen output and may omit the node labels. The
  plot is rendered in \ty{gnuplot}.
...
#+begin_src latex
  ...
  option (\ty{-v}) and four program-specific options: By default, we
...
#+begin_src latex
  ...
  to a postscript file (\ty{-p}) and give the postscript plot specific
  dimensions (\ty{-d}). The user can set the size of the plot on the
  screen (\ty{-S}), and the scale of the tree (\ty{-c}).
#+end_src
#+begin_src go <<Declare options, Ch.~\ref{ch:pt}>>=
  var optP = flag.String("p", "", "encapsulated postscript file")
  var optD = flag.String("d", "x,y", "dimensions of postscript plot " +
	  "in inch or cm")
  var optSS = flag.Float64("S", 0.9, "size of plot on screen")
...
#+begin_src latex
  ...
  the file names, a file counter, and the options as its own arguments.
#+end_src
...
#+begin_src latex
  ...
  construct the \ty{gnuplot} script, construct the command to run it,
  actually run it, and delete the script again.
#+end_src
#+begin_src go <<Draw tree, Ch.~\ref{ch:pt}>>=
  //<<Construct tree segments, Ch.~\ref{ch:pt}>>
  //<<Construct plot title, Ch.~\ref{ch:pt}>>
  //<<Construct script, Ch.~\ref{ch:pt}>>
  //<<Construct command, Ch.~\ref{ch:pt}>>
  //<<Run command, Ch.~\ref{ch:pt}>>
  //<<Delete script, Ch.~\ref{ch:pt}>>
#+end_src
#+begin_src latex
...
#+begin_src latex
  ...
  the label. To prevent a label from touching the point labeled, I pad
...
#+end_src
#+begin_src latex
  To construct the R script, we open a file, write the script to it, and
  close it again.
#+end_src
#+begin_src go <<Construct script, Ch.~\ref{ch:pt}>>=
  var script *os.File
...
#+end_src
#+begin_src latex
  We write the script in four steps: We write the terminal, remove the
  axes, write the labels, write the plot, and write its segments.
#+end_src
...
#+begin_src go <<Remove axes, Ch.~\ref{ch:pt}>>=
  fmt.Fprintf(script, "unset xtics\n")
  fmt.Fprintf(script, "unset ytics\n")
  fmt.Fprintf(script, "unset border\n")
#+end_src
#+begin_src latex
...
#+begin_src go <<Write labels, Ch.~\ref{ch:pt}>>=
  ...
  for _, s := range segments {
	  if s.l != "" {
		  ...
		  fmt.Fprintf(script, t, s.l,
			  s.o, a, s.x1, s.y1)
	  }
  }
#+end_src
#+begin_src latex
  Our plot is a line plot that might have a title. It has a size on the
  screen and an origin that is adjusted with respect to that size. For
  unrooted trees we adjust horizontally and vertically, for rooted trees
  just vertically.
#+end_src
#+begin_src go <<Write plot, Ch.~\ref{ch:pt}>>=
  ...
  fmt.Fprintf(script, "set size %.3f\n", opts.Size)
  xo := 0.0
  yo := (1.0 - opts.Size) / 2.0
  if opts.Unrooted { xo = yo }
  fmt.Fprintf(script, "set origin %.3f, %.3f\n", xo, yo)
  fmt.Fprintf(script, "plot \"-\" t \"\" w l lw 3\n")
#+end_src
#+begin_src latex
...
#+begin_src go <<Write segments, Ch.~\ref{ch:pt}>>=
  for i, s := range segments {
	  ...
	  fmt.Fprintf(script, "%.3f %.3f\n%.3f %.3f\n",
		  s.x1, s.y1, s.x2, s.y2)
  }
#+end_src
#+begin_src latex
  Having written the script, we run it.
#+end_src
#+begin_src go <<Construct command, Ch.~\ref{ch:pt}>>=
...
#+end_src
#+begin_src go <<Run command, Ch.~\ref{ch:pt}>>=
  err = cmd.Run()
  if err != nil {
...
#+end_src
#+begin_src latex
  All the trees have been printed, so we delete the script, unless the
  user asked for it.
#+end_src
#+begin_src go <<Delete script, Ch.~\ref{ch:pt}>>=
  if opts.Script == "" {
...
#+begin_src latex
  ...
  \ty{r2.gp}, and so on. On darwin systems, these names are extended by ``d''.
#+end_src
