...
#+begin_src go <<Set usage, Ch.~\ref{ch:pt}>>=
  ...
  e := "a) plotTree foo.nwk\n" +
	  "\t b) plotTree -S foo.r -d foo.nwk > foo.dat; " +
...
#+end_src
#+begin_src latex
  Finally, the user may opt to just print the R script and quit and/or
  print the raw data.
#+end_src
#+begin_src go <<Declare options, Ch.~\ref{ch:pt}>>=
  var optSS = flag.String("S", "", "write R script to file")
  var optD = flag.Bool("d", false, "print raw data")
#+end_src
#+begin_src latex
...
#+begin_src go <<Opts fields, Ch.~\ref{ch:pt}>>=
  Rooted, Unrooted, NoLabels, Data bool
  Ps string
...
#+begin_src latex
  ...
  We draw a tree by constructing its segments. If desired, we also print
  the segments. We then add margins and a title to the plot. Then we
  construct the R script and the R command to run it. The segments are
  piped into the R script and we run the R command. When we're done with
  the R script, we delete it.
#+end_src
#+begin_src go <<Draw tree, Ch.~\ref{ch:pt}>>=
  //<<Construct tree segments, Ch.~\ref{ch:pt}>>
  if options.Data {
	  //<<Print tree segments, Ch.~\ref{ch:pt}>>
  }
...
#+end_src
#+begin_src latex
  We print the tree segments to the standard output stream.
#+end_src
#+begin_src go <<Print tree segments, Ch.~\ref{ch:pt}>>=
...
#+end_src
#+begin_src latex
  Having written the R script, we run it.
#+end_src
