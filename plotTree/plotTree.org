...
#+begin_src latex
  ...
  specific dimensions (\ty{-d}). The user can set the relative size of
  the plot (\ty{-S}), and the scale of the tree (\ty{-c}).
#+end_src
#+begin_src go <<Declare options, Ch.~\ref{ch:pt}>>=
  var optP = flag.String("p", "", "encapsulated postscript file and exit")
...
#+begin_src latex
  ...
  construct the output stream. We write to the output stream in a Go
  routine, in which we also close the output stream again. Unless the
  user opted for the script, we run \ty{gnuplot}.
#+end_src
...
#+begin_src latex
  ...
  padded with a blank on the right hand side and we set the orientation
  to \emph{right}, \ty{r}.
#+end_src
...
#+begin_src latex
  ...
  remove the axes, write the labels, write the plot, and write its
  segments.
#+end_src
...
#+end_src
#+begin_src latex
  Our plot is a line plot in black that might have a title. It has a
  size on the screen and an origin that is adjusted with respect to that
  size. For unrooted trees we adjust horizontally and vertically, for
  rooted trees just vertically.
#+end_src
#+begin_src go <<Write plot, Ch.~\ref{ch:pt}>>=
  ...
  fmt.Fprintf(wr, "set size %.3f\n", opts.Size)
  xo := 0.0
  yo := (1.0 - opts.Size) / 2.0
  if opts.Unrooted { xo = yo }
  fmt.Fprintf(wr, "set origin %.3f, %.3f\n", xo, yo)
  fmt.Fprintf(wr, "plot \"-\" t \"\" w l lw 3 lc \"black\"\n")
#+end_src
#+begin_src latex
