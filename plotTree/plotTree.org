...
#+begin_src latex
  ...
  label. Now, \ty{gnuplot} renders a character preceded by an underscore
  as a subscript, which often looks confusing in the final tree. To
  preserve any underscores our label may contain, we prefix them with
  double backslashes, which we denote by their hexadecimal value. Then
  we copy the branch length and the tree topology.
#+end_src
#+begin_src go <<Functions, Ch.~\ref{ch:pt}>>=
...
#+end_src
#+begin_src latex
  We run tests on a tree where the labels don't contain underscores and
  on a tree where they do.
#+end_src
#+begin_src go <<Construct tests, Ch.~\ref{ch:pt}>>=
  //<<Tree without underscores, Ch.~\ref{ch:pt}>>
  //<<Tree with underscores, Ch.~\ref{ch:pt}>>
#+end_src
#+begin_src latex
  Our tests without underscores run on the Newick tree shown in
  Figure~\ref{fig:phy}A, which is stored in \ty{newick1.nwk}. We draw
  rooted and unrooted versions of it, with and without node labels.
#+end_src
#+begin_src go <<Tree without underscores, Ch.~\ref{ch:pt}>>=
  f := "newick1.nwk"
...
#+end_src
#+begin_src latex
  File \ty{newick2.nwk} contains the tree \verb+('A_1',B_2);+, where
  both labels contain an underscore. We expect that in the quoted label
  the underscore is rendered as underscore---not subscripting the next
  character---removed, while in the unquoted label the underscore is
  removed as specified by the Newick format. The result is shown in
  Figure~\ref{fig:nwk2}. For our test we plot the tree on the default
  terminal and on the ``dumb'' terminal. This
  \begin{figure}
    \begin{center}
      \includegraphics{../plotTree/newick2.ps}
    \end{center}
    \caption{Rendering the tree \ty{('A\_1',B\_2);} with a quoted and an
      unquoted label, both containing an underscore.}\label{fig:nwk2}
  \end{figure}
#+end_src
#+begin_src go <<Tree with underscores, Ch.~\ref{ch:pt}>>=
  f = "newick2.nwk"
