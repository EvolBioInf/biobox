#+begin_src latex
  ...
  \begin{enumerate}
    \item the \emph{suffix array} of alphabetically ordered suffixes,
      $\sa$
  ...
  containing set of all shustrings, which we call \emph{local}.

  the set of all shustrings
...
#+begin_src latex
  ...
  calculate the enhanced suffix array of the concatenated data. From the
  enhanced suffix array we compute the shustrings---strictly speaking
  their lengths---which are analyzed and printed.
#+end_src
