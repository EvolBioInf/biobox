#+begin_src latex
  ...
  t=\texttt{TATTTTTATA}
  \]
  consisting of ten nucleotides. At every position in $t$ we can ask,
  what is the shortest unique substring starting there? Consider the
  first nucleotide in $t$, $t[1...1]=\texttt{T}$ is not unique, nor is
  $t[1...2]=\texttt{TA}$ or $t[1...3]=\texttt{TAT}$, but
  $t[1...4]=\texttt{TATT}$ is. Since all extensions of \texttt{TATT},
  such as $t[1...5]=\texttt{TATTT}$, $t[1...6]=\texttt{TATTTT}$, and
  so on, are also unique, we call \texttt{TATT} emph{shortest} unique,
  a shustring.
  ...
  \begin{enumerate}
    \item the \emph{suffix array} of alphabetically suffixes,
      $\sa$
    \item the longest common prefix array of the lengths of matching
      prefixes, $\lcp$, and
    \item the inverse suffix array to map positions in $t$ onto
      positions in $\sa$.
  \end{enumerate}
  ...
  consists of three columns of integers that refer to the starting
  positions and lengths of strings. However, it is easier to think
  about strings by looking at them than by contemplating their
  positions and lengths, hence Table~\ref{tab:esa} also shows the
  suffixes, $\suf$, of $t$. Their starting positions are in $\sa$. The
  lengths of the prefixes matching between $\suf[i]$ and $\suf[i-1]$
  are in $\lcp[i]$. And $\isa$ stores the positions of suffixes in
  text-order. For example, the first suffix in $t$, $t[1...]$, is
  located at $\isa[1]=6$, the second suffix, $t[2...]$, at
  $\isa[2]=3$, and so on.

  From the enhanced suffix array in Table~\ref{tab:esa}, we can look
  up the shustring lengths in Table~\ref{tab:shu} as
  ...
  shustrings? Table~\ref{tab:shu} tells us that's 3, and there are
  three of them, \texttt{ATT}, \texttt{TTA}, and \texttt{ATA}. We call
  such shortest shustrings \emph{global}, the set of all shustrings
  \emph{local}. The program \texttt{shustring} computes either global
  or local shustrings for an arbitrary set of sequences.
...
#+end_src
#+begin_src latex
  We import \texttt{regexp}, \texttt{log}, and \texttt{util}.
#+end_src
#+begin_src go <<Imports, Ch.~\ref{ch:shu}>>=
  "regexp"
  "log"
...
#+begin_src latex
  ...
  one in turn. Scan takes as arguments a slice of pointers to sequences,
  the option values for local, reverse, and quiet, and the regular
  expression to identify sequences.
#+end_src
#+begin_src go <<Parse input files, Ch.~\ref{ch:shu}>>=
  files := flag.Args()
  var sequences []*fasta.Sequence
  clio.ParseFiles(files, scan, sequences,
	  *optL, *optR, *optQ, seqReg)
#+end_src
#+begin_src latex
...
#+begin_src go <<Retrieve arguments, Ch.~\ref{ch:shu}>>=
  sequences := args[0].([]*fasta.Sequence)
  local := args[1].(bool)
  reverse := args[2].(bool)
  quiet := args[3].(bool)
  seqReg := args[4].(*regexp.Regexp)
#+end_src
#+begin_src latex
  The sequences contained in the current file are stored in a slice.
#+end_src
...
#+begin_src latex
  ...
  calculate the enhanced suffix array for the sequence data. From the
  enhanced suffix array we compute the shustrings---strictly speaking
  their lengths. These are analyzed and printed.
#+end_src
...
#+begin_src latex
  ...
  positions. However, concatenation can create new substrings that mask
  legitimate shustrings. Consider for example the two sequences
  $s_1=\texttt{GTG}$ and $s_2=\texttt{TT}$. Their combined shustring
  inventory is \{\texttt{GT}, \texttt{TG}, and \texttt{TT}. However,
  concatenation to \texttt{GTGTT} creates a second \texttt{GT}, which
  masks the uniqueness of the first. To prevent this, we separate
  sequences by a character outside of their alphabet, the zero byte.
#+end_src
...
#+begin_src latex
  ...
  suffix array proper, the longest common prefix array, and the inverse
  suffix array.
#+end_src
#+begin_src go <<Compute enhanced suffix array, Ch.~\ref{ch:shu}>>=
  sa := esa.Sa(cat)
  lcp, isa := esa.LcpIsa(cat, sa)
#+end_src
#+begin_src latex
  We import \texttt{esa}.
#+end_src
...
#+begin_src latex
  ...
  equation~(\ref{eq:shu}).  To ensure that there always is an element
  $\lcp[i+1]$, we append a value smaller than any legitimate prefix
  length to $\lcp$, -1.
#+end_src
...
#+end_src
#+begin_src latex
  When calculating an individual shustring length, we set shustrings
  that extend beyond the end of their host sequence to the largest
  integer available.
#+end_src
...
#+end_src
#+begin_src latex
  In local mode we print all shustrings, in global just the shortest. So
  the difference between these modes is the maximum length of a
  shustring to be printed. In local mode, we include all shustrings and
  thus set the maximum is a very large integer, but not the largest, as
  we have just used that to mark positions without any shustring at
  all. In global mode, the minimum shustring length becomes the
  maximum, which is computed for each sequence.
#+end_src
...
#+end_src
#+begin_src latex
  The table header differs between the modes. In global mode, it consists
  of four fields:
  \begin{tabular}{cccc}
    count & position & length & sequence
  \end{tabular}
  In local mode, the count and the position coincide, so we drop the
...
#+begin_src latex
  ...
  and count and write the shustrings that are no longer than the
  maximum. 
#+end_src
...
#+end_src
#+begin_src latex
  We run six tests, one for each option plus one without any options
#+end_src
