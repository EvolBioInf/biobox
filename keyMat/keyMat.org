...
#+end_src
#+begin_src latex
  Apart from the default help option, \texttt{-h}, we declare four
  additional options:
  \begin{enumerate}
  \item \texttt{-p}: one or more patterns
...
#+end_src
#+begin_src latex
  The patterns are collected from the command line and/or a file. Since
  the file contains FASTA-formatted sequences, all patterns are stored
  in this format.
#+end_src
...
#+end_src
#+begin_src latex
  When running the search, we iterate over the input files and apply the
  function \texttt{scan} to each one. This function takes as argument
  the keyword tree constructed from the patterns, the patterns in
  sequence and string format, and the \texttt{-r} flag.
#+end_src
...
#+begin_src go <<Write commands, Ch.~\ref{ch:km}>>=
  c := exec.Command(r, "-p",  p, i)
  commands = append(commands, c)
  c = exec.Command(r, "-p", p, "-r", i)
  commands = append(commands, c)
  c = exec.Command(r, "-f", f, i)
  commands = append(commands, c)
  c = exec.Command(r, "-f", f, "-r", i)
  commands = append(commands, c)
  c = exec.Command(r, "-f", f, "-p", "ATTA", i)
  commands = append(commands, c)
#+end_src
#+begin_src latex
