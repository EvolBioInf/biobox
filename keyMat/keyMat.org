...
#+begin_src latex
  ...
  \item \texttt{-o}: omit outut sets for didactical purposes
...
#+begin_src latex
  ...
  command line is assumed to contain the patterns and is lopped off the
  beginning of the list of input files.
#+end_src
#+begin_src go <<Collect patterns, Ch.~\ref{ch:km}>>=
...
#+end_src
#+begin_src latex
  When running the search, we first construct the slice of pattern
  strings and then the keyword tree from them. Then we iterate over the
  input files, which are the remaining command line arguments. We apply
  the function \texttt{scan} to each file. This function takes as
  argument the keyword tree constructed from the patterns, the patterns
  in the sequence format as we just stored them and in string format,
  the \texttt{-r} flag, and the \ty{-i} flag. If the user opted to
  ignore case (\ty{-i}), we set all patterns in the keyword tree to
  upper case, and later do the same thing with the sequences.
#+end_src
#+begin_src go <<Run search, Ch.~\ref{ch:km}>>=
  var tree *kt.Node
  var pStrings []string
  //<<Construct slice of pattern strings, Ch.~\ref{ch:km}>>
  //<<Construct keyword tree, Ch.~\ref{ch:km}>>
  clio.ParseFiles(files, scan, tree, patterns, pStrings, *optR,
	  (*optI))
#+end_src
#+begin_export latex
When constructing the slice of patterns, we set them to upper case if
the user decided to ignore case.
#+end_export
#+begin_src go <<Construct slice of pattern strings, Ch.~\ref{ch:km}>>=
...
#+end_src
#+begin_export latex
We construct a new keyword tree by calling the corresponding function
in \ty{kt}. If the user opted to omit the output sets, we also call
the function \ty{omit\-Output\-Sets}, which we still need to write.
#+end_export
#+begin_src go <<Construct keyword tree, Ch.~\ref{ch:km}>>=
...
#+end_src
#+begin_export latex
In the function \ty{omitOutputSets} we recursively traverse the
keyword tree. Any non-empty output set we find on the way is reduced
to its first entry. This recovers the state of the keyword tree before
the output sets were constructed.
#+end_export
